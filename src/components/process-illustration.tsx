...
const SEARCH_QUERIES = [
  { term: "boiler installation near me", trade: "Heating & Gas", area: "Local area" },
  { term: "emergency roofer near me", trade: "Roofing Specialist", area: "Local area" },
  { term: "bathroom renovation near me", trade: "Bathrooms & Tiling", area: "Local area" },
  { term: "commercial electrician near me", trade: "Electrician", area: "Local area" },
]
...
export function ProcessIllustration() {
  ...
  return (
    <figure
      ...
      className="relative mx-auto w-full max-w-lg rounded-3xl border border-primary/20 bg-linear-to-b from-white via-slate-50/50 to-primary/5 p-5 sm:p-7 shadow-[0_12px_36px_rgba(37,99,235,0.08)]"
      aria-labelledby="process-illustration-title"
    >
      <figcaption id="process-illustration-title" className="sr-only">
        Illustrative example of a search, advert and enquiry. No real campaign is shown.
      </figcaption>

      {/* Floating Sparkle / Hand-drawn Accents */}
      <div className="absolute -top-3 -right-3 flex items-center gap-1.5 rounded-full border border-primary/20 bg-white px-3 py-1 text-xs font-semibold text-primary shadow-sm">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between text-xs font-medium text-muted-foreground">
          <span className="flex items-center gap-1.5">
            ...
            Homeowner search intent
          </span>
          <span className="text-[0.7rem] text-slate-400">Step 01</span>
        </div>
      ...
      <div className="space-y-2.5 rounded-2xl border border-border bg-white p-4 shadow-xs">
        <div className="flex items-center justify-between text-xs">
          <span className="text-[0.7rem] text-muted-foreground font-mono">example-trade.com</span>
          <span className="text-[0.7rem] text-slate-400">Step 02</span>
        </div>
        ...
        <div>
          ...
          >
            Local {currentQuery.trade} in your area & Local area
            <ExternalLink className="size-3 text-primary/60 inline" />
          </motion.p>
          <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
            Example advert for {currentQuery.area}. Replace this copy with claims your business can support.
          </p>
        </div>
      ...
      <div className="space-y-3 rounded-2xl border border-primary/20 bg-linear-to-r from-blue-500/10 via-primary/5 to-white p-4 shadow-xs">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-[0.7rem] text-slate-400">Step 03</span>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl bg-white p-3 border border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold text-foreground">Example Call</p>
              <p className="text-[0.7rem] text-muted-foreground">Caller area: {currentQuery.area}</p>
            </div>
          </div>
          <div className="text-right">
            ...
            <p className="text-[0.62rem] text-muted-foreground mt-0.5 font-semibold text-emerald-600">EXAMPLE</p>
          </div>
        </div>

        <div className="flex items-center justify-between text-[0.7rem] text-muted-foreground pt-0.5">
          <span className="italic">“Example customer enquiry”</span>
